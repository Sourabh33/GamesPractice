...
public class Sudoku {
    ...
    public static final int GAME_MODE_EASIER = 4;
    ...
    private int[][] swapColumns(int[][] board, int col1, int col2) {
        for (int i = 0; i < board.length; i++) {
    ...
    private int[][] swapRowsAndColumns(int[][] board) {
        int range = board.length == GRID_9X9 ? 7 : 5;
        int rowsInGrid = board.length == GRID_9X9 ? 3 : 2;
        int columsInGrid = 3;

        for (int a = 0; a < range; a += rowsInGrid) {
            int[] row = getTwoRandomNumber(a, rowsInGrid);
            swapRows(board, row[0], row[1]);
        }

        for (int a = 0; a < range; a += columsInGrid) {
            int[] col = getTwoRandomNumber(a, columsInGrid);
            swapColumns(board, col[0], col[1]);
        }
        return board;
    }

    private int[][] swapGrid(int[][] board) {
    ...
    private int[][] swapNums(int[][] board) {
        int[] num = getTwoRandomNumber(1, board.length);
        for (int i = 0; i < board.length; i++) {
    ...
    private int[] getTwoRandomNumber(int min, int tolerance) {
        int[] a = new int[2];
    ...
    private int[][] createPuzzle(int[][] board, int mode) {
        ...
        for (int i = 0; i < numOfEmptyBlock; i++) {
            int[] rowCol = getTwoRandomNumber(0, board.length);
    ...
    private int getNumberOfEmptyBlock(int[][] board, int mode) {
        ...
        if (GAME_MODE_EASIER <= mode && mode <= GAME_MODE_EXPERT) {
